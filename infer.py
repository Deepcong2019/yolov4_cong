...
def get_anchors_and_decode(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    #   K.arange创建包含整数序列的1D张量。K.arange(0, stop=13),
    #   <tf.Tensor 'arange:0' shape=(13,) dtype=int32>
    #   K.reshape(K.arange(0, stop=13), [1, -1, 1, 1]),
    #   <tf.Tensor 'Reshape:0' shape=(1, 13, 1, 1) dtype=int32>
    #   grid_x = K.tile(K.reshape(K.arange(0, stop=13), [1, -1, 1, 1]),[13,1,3,1]),
    #   <tf.Tensor 'Tile:0' shape=(13, 13, 3, 1) dtype=int32>
    #   grid_y = K.tile(K.reshape(K.arange(0, 13), [-1, 1, 1, 1]), [1, 13, 3, 1])
    #   <tf.Tensor 'Tile_4:0' shape=(13, 13, 3, 1) dtype=int32>
    #   K.tile(x,n) 将x在各个维度上重复n次，x为张量，n为与x维度数目相同的列表
    #   K.concatenate([grid_x, grid_y])
    #   <tf.Tensor 'concat_1:0' shape=(13, 13, 3, 2) dtype=int32>
    # --------------------------------------------------------------------#
    ...
    #    <tf.Tensor 'Reshape_6:0' shape=(1, 1, 3, 2) dtype=float32>
    anchors_tensor = K.tile(anchors_tensor, [grid_shape[0], grid_shape[1], 1, 1])
    #    <tf.Tensor 'Tile_5:0' shape=(13, 13, 3, 2) dtype=float32>
    ...
    # feats: Tensor("conv2d_110/BiasAdd:0", shape=(?, ?, ?, 21), dtype=float32)-->
    #     -->Tensor("Reshape_11:0", shape=(?, ?, ?, 3, 7), dtype=float32)
    #        7: x,y,w,h,confidence,classprobs
    # ------------------------------------------#
    #   对先验框进行解码，并进行归一化
    # ------------------------------------------#
    # grid :shape = (13, 13, 3, 2)
...
def DecodeBox(outputs,
    ...
    for c in range(num_classes):
        ...
        nms_index = tf.image.non_max_suppression(class_boxes, class_box_scores,
                                                 max_boxes_tensor, iou_threshold=nms_iou)
        #-----------------------------------------------------------#
        #   获取非极大抑制后的结果
        #   下列三个分别是：框的位置，得分与种类
        #-----------------------------------------------------------#
...
print('model:', model)
print('model_layers_num:', len(model.layers))

#tf的方式计算结果
input_image_shape = K.placeholder(shape=(2, ))
sess = K.get_session()
boxes, scores, classes = DecodeBox(model.output, anchors, num_classes, input_image_shape, input_shape, anchors_mask,
                                     max_boxes, confidence, nms_iou, letterbox_image)
result = sess.run([boxes, scores, classes], feed_dict={model.input: image_data, input_image_shape: [image.size[1], image.size[0]]})
